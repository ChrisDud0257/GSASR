...
def main(args):
    ...
    final_save_path = os.path.join(args.save_sr_path, args.model, f"x{args.scale}")
    ...
    for img in img_list:
        ...
        cv2.imwrite(os.path.join(final_save_path, f'{imgname}_{args.model}{ext}'), output)
        print(f"Save the SR image to {os.path.join(final_save_path, f'{imgname}_{args.model}{ext}')}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_img_path', type=str, default='./DIV2K100_GT720/x4/LR', help='input test image folder')
    parser.add_argument('--save_sr_path', type=str, default='./output_paper_benchamrk', help='output folder')
